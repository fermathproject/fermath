...
public:
    ...
    void add_basic_unit(const basic_unit &bunit,magnitude_id magid) {
        basic_unit_id bid;
        ...
        add_complex_unit(unit2,magid); //the unit is added to complex source
    }
    //add a complex unit
    ...
    complex_unit search(unit_id p) const {
        complex_unit result;
        magnitude m=search_magnitude(p.first);
    ...
    magnitude_id search_magnitude(string n) const {
        map<magnitude_id,magnitude>::const_iterator it;
        magnitude_id res=0;
        bool found=false;
